# Test 110: Socket Quest Edge Cases

> **Layer P127** (Depth 5) — Full Integration / Edge Cases
> **Error Code:** `P127_E110`
> **Screen:** Integration
> **Run:** `index.html?src=tests/d2r-oracle/test-110-edge-socket-quest.md`

## Description

Larzuk quest on different item types

## Query Input

```
larzuk normal crystal sword vs hell
```

## Expected Result

```
Normal=4os guaranteed, varies by ilvl
```
...
```js
/**
 * D2R Oracle Test — Socket Quest Edge Cases
 * Layer: P127 | Error Code: P127_E110
...
function main(params) {
  const testId = '110';
  const errorCode = 'P127_E110';
  const testName = 'edge-socket-quest';
  const layer = { prime: 127, depth: 5, name: 'Full Integration / Edge Cases' };
  const testSrc = 'tests/d2r-oracle/test-110-edge-socket-quest.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Socket Quest Edge Cases');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[110] Running integration test');
    const query = "larzuk normal crystal sword vs hell";
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[110] Integration test passed');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'Normal=4os guaranteed, varies by ilvl';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-109-edge-ethereal-bug.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 109</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-111-integ-build-to-farm.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 111 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 110</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P127 \u00B7 Depth 5</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Socket Quest Edge Cases</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Larzuk quest on different item types</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P127_E110</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">larzuk normal crystal sword vs hell</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Normal=4os guaranteed, varies by ilvl</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-110-edge-socket-quest.md`*
