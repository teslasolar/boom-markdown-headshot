# Test 073: Item Lookup: Tal Rasha Set

> **Layer P31** (Depth 4) — Query Screens
> **Error Code:** `P31_E073`
> **Screen:** ItemLookup
> **Run:** `index.html?src=tests/d2r-oracle/test-073-screen-item-set-tals.md`

## Description

Tal Rasha full set with partial bonuses
...
```
All 5 pieces + partial bonuses + full bonus
```
...
```js
/**
 * D2R Oracle Test — Item Lookup: Tal Rasha Set
 * Layer: P31 | Error Code: P31_E073
...
function main(params) {
  const testId = '073';
  const errorCode = 'P31_E073';
  const testName = 'screen-item-set-tals';
  const layer = { prime: 31, depth: 4, name: 'Query Screens' };
  const testSrc = 'tests/d2r-oracle/test-073-screen-item-set-tals.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Item Lookup: Tal Rasha Set');
    ...
    // Item lookup screen
    if (typeof MDRUN !== 'undefined') MDRUN.info('[073] Testing item lookup: tal rasha set');
    const query = "tal rasha set";
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[073] Item lookup screen verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'All 5 pieces + partial bonuses + full bonus';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-072-screen-item-anni.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 072</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-074-screen-item-cta.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 074 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 073</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P31 \u00B7 Depth 4</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Item Lookup: Tal Rasha Set</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Tal Rasha full set with partial bonuses</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P31_E073</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">tal rasha set</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">All 5 pieces + partial bonuses + full bonus</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-073-screen-item-set-tals.md`*
