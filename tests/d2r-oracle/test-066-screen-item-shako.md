# Test 066: Item Lookup: Shako

> **Layer P31** (Depth 4) — Query Screens
> **Error Code:** `P31_E066`
> **Screen:** ItemLookup
> **Run:** `index.html?src=tests/d2r-oracle/test-066-screen-item-shako.md`

## Description

Full Shako lookup with stats, farm spots, value
...
```
Harlequin Crest full tooltip + farm + value
```
...
```js
/**
 * D2R Oracle Test — Item Lookup: Shako
 * Layer: P31 | Error Code: P31_E066
...
function main(params) {
  const testId = '066';
  const errorCode = 'P31_E066';
  const testName = 'screen-item-shako';
  const layer = { prime: 31, depth: 4, name: 'Query Screens' };
  const testSrc = 'tests/d2r-oracle/test-066-screen-item-shako.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Item Lookup: Shako');
    ...
    // Item lookup screen
    if (typeof MDRUN !== 'undefined') MDRUN.info('[066] Testing item lookup: what is shako');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[066] Item lookup screen verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'Harlequin Crest full tooltip + farm + value';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-065-calc-trade-value-ist.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 065</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-067-screen-item-enigma.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 067 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 066</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P31 \u00B7 Depth 4</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Item Lookup: Shako</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Full Shako lookup with stats, farm spots, value</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P31_E066</code></div>' +
        '</div>' +
        ...
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          ...
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Harlequin Crest full tooltip + farm + value</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-066-screen-item-shako.md`*
