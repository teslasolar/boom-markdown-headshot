# Test 056: FCR Breakpoints Sorceress

> **Layer P11** (Depth 3) — Calculators
> **Error Code:** `P11_E056`
> **Screen:** Breakpoints
> **Run:** `index.html?src=tests/d2r-oracle/test-056-calc-fcr-sorc.md`

## Description

All FCR breakpoints: 9/20/37/63/105/200
...
```
0%=13f, 9%=12f, 20%=11f, 37%=10f, 63%=9f, 105%=8f, 200%=7f
```
...
```js
/**
 * D2R Oracle Test — FCR Breakpoints Sorceress
 * Layer: P11 | Error Code: P11_E056
...
function main(params) {
  const testId = '056';
  const errorCode = 'P11_E056';
  const testName = 'calc-fcr-sorc';
  const layer = { prime: 11, depth: 3, name: 'Calculators' };
  const testSrc = 'tests/d2r-oracle/test-056-calc-fcr-sorc.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: FCR Breakpoints Sorceress');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[056] Validating breakpoint tables');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[056] Breakpoint tables verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = '0%=13f, 9%=12f, 20%=11f, 37%=10f, 63%=9f, 105%=8f, 200%=7f';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-055-calc-merc-damage.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 055</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-057-calc-fcr-paladin.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 057 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 056</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P11 \u00B7 Depth 3</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">FCR Breakpoints Sorceress</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">All FCR breakpoints: 9/20/37/63/105/200</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P11_E056</code></div>' +
        '</div>' +
        ...
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          ...
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">0%=13f, 9%=12f, 20%=11f, 37%=10f, 63%=9f, 105%=8f, 200%=7f</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-056-calc-fcr-sorc.md`*
