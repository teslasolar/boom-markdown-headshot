# Test 006: API Key Storage

> **Layer P2** (Depth 0) — Foundation / Smoke
> **Error Code:** `P2_E006`
> **Screen:** CLI
> **Run:** `index.html?src=tests/d2r-oracle/test-006-smoke-api-key-set.md`

## Description

Verify --set-key stores and retrieves key

## Query Input

```
d2r --set-key sk-ant-test
```

## Expected Result

```
Key stored securely, confirmation shown
```
...
```js
/**
 * D2R Oracle Test — API Key Storage
 * Layer: P2 | Error Code: P2_E006
...
function main(params) {
  const testId = '006';
  const errorCode = 'P2_E006';
  const testName = 'smoke-api-key-set';
  const layer = { prime: 2, depth: 0, name: 'Foundation / Smoke' };
  const testSrc = 'tests/d2r-oracle/test-006-smoke-api-key-set.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: API Key Storage');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[006] MDRUN runtime detected');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[006] CLI entry point verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'Key stored securely, confirmation shown';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-005-smoke-offline-mode.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 005</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-007-smoke-output-format.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 007 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 006</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P2 \u00B7 Depth 0</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">API Key Storage</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Verify --set-key stores and retrieves key</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Screen:</strong> <span style="color: #e5e7eb;">CLI</span></div>' +
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P2_E006</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">d2r --set-key sk-ant-test</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Key stored securely, confirmation shown</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-006-smoke-api-key-set.md`*
