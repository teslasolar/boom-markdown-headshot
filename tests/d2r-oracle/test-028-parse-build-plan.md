# Test 028: Parse Build Plan Query

> **Layer P5** (Depth 2) — Parser / NLP
> **Error Code:** `P5_E028`
> **Screen:** Parser
> **Run:** `index.html?src=tests/d2r-oracle/test-028-parse-build-plan.md`

## Description

NLP identifies "budget hammerdin" as BUILD_PLAN
...
```
type: BUILD_PLAN, build: "hammerdin", budget: "low"
```
...
```js
/**
 * D2R Oracle Test — Parse Build Plan Query
 * Layer: P5 | Error Code: P5_E028
...
function main(params) {
  const testId = '028';
  const errorCode = 'P5_E028';
  const testName = 'parse-build-plan';
  const layer = { prime: 5, depth: 2, name: 'Parser / NLP' };
  const testSrc = 'tests/d2r-oracle/test-028-parse-build-plan.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Parse Build Plan Query');
    ...
    const query = "budget hammerdin";
    if (typeof MDRUN !== 'undefined') MDRUN.info('[028] Parsing query: ' + JSON.stringify(query));
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[028] Parser classification verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'type: BUILD_PLAN, build: "hammerdin", budget: "low"';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-027-parse-drop-calc.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 027</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-029-parse-runeword-finder.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 029 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 028</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P5 \u00B7 Depth 2</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Parse Build Plan Query</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">NLP identifies "budget hammerdin" as BUILD_PLAN</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P5_E028</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">budget hammerdin</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">type: BUILD_PLAN, build: "hammerdin", budget: "low"</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-028-parse-build-plan.md`*
