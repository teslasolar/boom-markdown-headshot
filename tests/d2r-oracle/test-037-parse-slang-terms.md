# Test 037: Parse D2R Slang

> **Layer P5** (Depth 2) — Parser / NLP
> **Error Code:** `P5_E037`
> **Screen:** Parser
> **Run:** `index.html?src=tests/d2r-oracle/test-037-parse-slang-terms.md`

## Description

Handle slang like "hoto", "cta", "bo", "tp"
...
```
Resolves to "Heart of the Oak" runeword
```
...
```js
/**
 * D2R Oracle Test — Parse D2R Slang
 * Layer: P5 | Error Code: P5_E037
...
function main(params) {
  const testId = '037';
  const errorCode = 'P5_E037';
  const testName = 'parse-slang-terms';
  const layer = { prime: 5, depth: 2, name: 'Parser / NLP' };
  const testSrc = 'tests/d2r-oracle/test-037-parse-slang-terms.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Parse D2R Slang');
    ...
    const query = "hoto";
    if (typeof MDRUN !== 'undefined') MDRUN.info('[037] Parsing query: ' + JSON.stringify(query));
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[037] Parser classification verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'Resolves to "Heart of the Oak" runeword';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-036-parse-ambiguous-item.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 036</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-038-parse-multi-intent.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 038 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 037</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P5 \u00B7 Depth 2</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Parse D2R Slang</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Handle slang like "hoto", "cta", "bo", "tp"</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P5_E037</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">hoto</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Resolves to "Heart of the Oak" runeword</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-037-parse-slang-terms.md`*
