# Test 088: Build: Fury Druid

> **Layer P31** (Depth 4) — Query Screens
> **Error Code:** `P31_E088`
> **Screen:** BuildPlan
> **Run:** `index.html?src=tests/d2r-oracle/test-088-screen-build-fury-druid.md`
...
```js
/**
 * D2R Oracle Test — Build: Fury Druid
 * Layer: P31 | Error Code: P31_E088
...
function main(params) {
  const testId = '088';
  const errorCode = 'P31_E088';
  const testName = 'screen-build-fury-druid';
  const layer = { prime: 31, depth: 4, name: 'Query Screens' };
  const testSrc = 'tests/d2r-oracle/test-088-screen-build-fury-druid.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Build: Fury Druid');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[088] Testing build planner: fury druid');
    const builds = {
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[088] Build planner verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'Skills + Ribcracker/Grief + IAS';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-087-screen-build-trapsin.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 087</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-089-screen-build-summoner.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 089 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 088</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P31 \u00B7 Depth 4</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Build: Fury Druid</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Fury werewolf druid</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P31_E088</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">fury druid</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Skills + Ribcracker/Grief + IAS</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-088-screen-build-fury-druid.md`*
