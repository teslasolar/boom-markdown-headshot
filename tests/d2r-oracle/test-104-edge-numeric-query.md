...
> **Screen:** Integration
> **Run:** `index.html?src=tests/d2r-oracle/test-104-edge-numeric-query.md`
...
function main(params) {
  ...
  const testSrc = 'tests/d2r-oracle/test-104-edge-numeric-query.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Numeric Only Query');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[104] Running integration test');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[104] Integration test passed');
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-103-edge-very-long-query.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 103</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-105-edge-nonexistent-item.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 105 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 104</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P127 \u00B7 Depth 5</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Numeric Only Query</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Handle "4os eth thresher" mixed numeric/text</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P127_E104</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">4os eth thresher</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Parsed as base item search with socket filter</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-104-edge-numeric-query.md`*
