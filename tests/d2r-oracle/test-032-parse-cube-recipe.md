# Test 032: Parse Cube Recipe Query

> **Layer P5** (Depth 2) — Parser / NLP
> **Error Code:** `P5_E032`
> **Screen:** Parser
> **Run:** `index.html?src=tests/d2r-oracle/test-032-parse-cube-recipe.md`

## Description

NLP identifies "upgrade um to ber" as CUBE_RECIPE
...
```
type: CUBE_RECIPE, from: "Um", to: "Ber"
```
...
```js
/**
 * D2R Oracle Test — Parse Cube Recipe Query
 * Layer: P5 | Error Code: P5_E032
...
function main(params) {
  const testId = '032';
  const errorCode = 'P5_E032';
  const testName = 'parse-cube-recipe';
  const layer = { prime: 5, depth: 2, name: 'Parser / NLP' };
  const testSrc = 'tests/d2r-oracle/test-032-parse-cube-recipe.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Parse Cube Recipe Query');
    ...
    const query = "upgrade um to ber";
    if (typeof MDRUN !== 'undefined') MDRUN.info('[032] Parsing query: ' + JSON.stringify(query));
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[032] Parser classification verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'type: CUBE_RECIPE, from: "Um", to: "Ber"';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-031-parse-merc-advice.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 031</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-033-parse-uber-guide.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 033 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 032</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P5 \u00B7 Depth 2</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Parse Cube Recipe Query</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">NLP identifies "upgrade um to ber" as CUBE_RECIPE</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P5_E032</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">upgrade um to ber</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">type: CUBE_RECIPE, from: "Um", to: "Ber"</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-032-parse-cube-recipe.md`*
