# Test 103: Very Long Query String

> **Layer P127** (Depth 5) — Full Integration / Edge Cases
> **Error Code:** `P127_E103`
> **Screen:** Integration
> **Run:** `index.html?src=tests/d2r-oracle/test-103-edge-very-long-query.md`

## Description

Handle 500+ character query gracefully
...
```
Truncation or helpful error, no crash
```
...
```js
/**
 * D2R Oracle Test — Very Long Query String
 * Layer: P127 | Error Code: P127_E103
...
function main(params) {
  const testId = '103';
  const errorCode = 'P127_E103';
  const testName = 'edge-very-long-query';
  const layer = { prime: 127, depth: 5, name: 'Full Integration / Edge Cases' };
  const testSrc = 'tests/d2r-oracle/test-103-edge-very-long-query.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Very Long Query String');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[103] Running integration test');
    const query = "";
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[103] Integration test passed');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'Truncation or helpful error, no crash';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-102-edge-special-chars.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 102</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-104-edge-numeric-query.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 104 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 103</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P127 \u00B7 Depth 5</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Very Long Query String</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Handle 500+ character query gracefully</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P127_E103</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;"></code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Truncation or helpful error, no crash</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-103-edge-very-long-query.md`*
