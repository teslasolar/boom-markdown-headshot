...
> **Error Code:** `P11_E043`
> **Screen:** DropCalc
> **Run:** `index.html?src=tests/d2r-oracle/test-043-calc-tc-resolution.md`
...
function main(params) {
  ...
  const testSrc = 'tests/d2r-oracle/test-043-calc-tc-resolution.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Treasure Class Resolution');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[043] Running drop calculator formulas');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[043] Drop calculator formulas verified');
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-042-calc-nodrop-players.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 042</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-044-calc-countess-rune-special.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 044 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 043</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P11 \u00B7 Depth 3</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Treasure Class Resolution</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Walk TC tree from monster to item-level TC</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P11_E043</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">Hell Mephisto Shako drop rate</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">~1:345 at 300MF</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-043-calc-tc-resolution.md`*
