# Test 052: Blizzard Sorceress DPS

> **Layer P11** (Depth 3) — Calculators
> **Error Code:** `P11_E052`
> **Screen:** DamageCalc
> **Run:** `index.html?src=tests/d2r-oracle/test-052-calc-blizz-sorc-dps.md`

## Description

Blizzard damage with synergies and cold mastery

## Expected Result

```
Blizzard damage with cold mastery penetration
```
...
```js
/**
 * D2R Oracle Test — Blizzard Sorceress DPS
 * Layer: P11 | Error Code: P11_E052
...
function main(params) {
  const testId = '052';
  const errorCode = 'P11_E052';
  const testName = 'calc-blizz-sorc-dps';
  const layer = { prime: 11, depth: 3, name: 'Calculators' };
  const testSrc = 'tests/d2r-oracle/test-052-calc-blizz-sorc-dps.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Blizzard Sorceress DPS');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[052] Testing damage calculations');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[052] Damage calculation verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'Blizzard damage with cold mastery penetration';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-051-calc-hammerdin-dps.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 051</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-053-calc-javazon-dps.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 053 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 052</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P11 \u00B7 Depth 3</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Blizzard Sorceress DPS</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Blizzard damage with synergies and cold mastery</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P11_E052</code></div>' +
        '</div>' +
        
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Blizzard damage with cold mastery penetration</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-052-calc-blizz-sorc-dps.md`*
