# Test 113: Item Eval Includes Trade

> **Layer P127** (Depth 5) — Full Integration / Edge Cases
> **Error Code:** `P127_E113`
> **Screen:** Integration
> **Run:** `index.html?src=tests/d2r-oracle/test-113-integ-eval-to-trade.md`

## Description

Item evaluation includes trade value

## Query Input

```
is 35fcr spirit worth keeping
```

## Expected Result

```
Max roll + trade value + keep vs sell
```
...
```js
/**
 * D2R Oracle Test — Item Eval Includes Trade
 * Layer: P127 | Error Code: P127_E113
...
function main(params) {
  const testId = '113';
  const errorCode = 'P127_E113';
  const testName = 'integ-eval-to-trade';
  const layer = { prime: 127, depth: 5, name: 'Full Integration / Edge Cases' };
  const testSrc = 'tests/d2r-oracle/test-113-integ-eval-to-trade.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Item Eval Includes Trade');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[113] Running integration test');
    const query = "is 35fcr spirit worth keeping";
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[113] Integration test passed');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'Max roll + trade value + keep vs sell';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-112-integ-farm-to-runeword.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 112</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-114-integ-breakpoint-to-gear.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 114 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 113</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P127 \u00B7 Depth 5</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Item Eval Includes Trade</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Item evaluation includes trade value</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P127_E113</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">is 35fcr spirit worth keeping</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Max roll + trade value + keep vs sell</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-113-integ-eval-to-trade.md`*
