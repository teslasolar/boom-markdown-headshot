# Test 024: Runes Cross-Reference Runewords

> **Layer P3** (Depth 1) — Data Integrity
> **Error Code:** `P3_E024`
> **Screen:** Data
> **Run:** `index.html?src=tests/d2r-oracle/test-024-data-cross-ref-runes-runewords.md`

## Description

Every rune in runewords.json exists in runes.json

## Expected Result

```
No unknown runes in runeword recipes
```
...
```js
/**
 * D2R Oracle Test — Runes Cross-Reference Runewords
 * Layer: P3 | Error Code: P3_E024
...
function main(params) {
  const testId = '024';
  const errorCode = 'P3_E024';
  const testName = 'data-cross-ref-runes-runewords';
  const layer = { prime: 3, depth: 1, name: 'Data Integrity' };
  const testSrc = 'tests/d2r-oracle/test-024-data-cross-ref-runes-runewords.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Runes Cross-Reference Runewords');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[024] Validating data file manifest');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[024] Data manifest validated: ' + dataFiles.length + ' files');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'No unknown runes in runeword recipes';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-023-data-cross-ref-tc-monsters.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 023</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-025-data-cross-ref-areas-monsters.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 025 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 024</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P3 \u00B7 Depth 1</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Runes Cross-Reference Runewords</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Every rune in runewords.json exists in runes.json</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P3_E024</code></div>' +
        '</div>' +
        
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">No unknown runes in runeword recipes</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-024-data-cross-ref-runes-runewords.md`*
