# Test 117: MF vs Clear Speed Analysis

> **Layer P127** (Depth 5) — Full Integration / Edge Cases
> **Error Code:** `P127_E117`
> **Screen:** Integration
> **Run:** `index.html?src=tests/d2r-oracle/test-117-integ-mf-vs-clearspeed.md`

## Description

Integrated analysis of MF efficiency
...
```
Math showing 300 MF sweet spot
```
...
```js
/**
 * D2R Oracle Test — MF vs Clear Speed Analysis
 * Layer: P127 | Error Code: P127_E117
...
function main(params) {
  const testId = '117';
  const errorCode = 'P127_E117';
  const testName = 'integ-mf-vs-clearspeed';
  const layer = { prime: 127, depth: 5, name: 'Full Integration / Edge Cases' };
  const testSrc = 'tests/d2r-oracle/test-117-integ-mf-vs-clearspeed.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: MF vs Clear Speed Analysis');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[117] Running integration test');
    const query = "is 500 mf worth it";
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[117] Integration test passed');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'Math showing 300 MF sweet spot';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-116-integ-cube-chain.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 116</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-118-integ-progression-path.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 118 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 117</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P127 \u00B7 Depth 5</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">MF vs Clear Speed Analysis</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Integrated analysis of MF efficiency</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P127_E117</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">is 500 mf worth it</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Math showing 300 MF sweet spot</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-117-integ-mf-vs-clearspeed.md`*
