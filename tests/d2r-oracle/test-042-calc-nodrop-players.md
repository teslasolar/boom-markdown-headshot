# Test 042: NoDrop with Players Setting

> **Layer P11** (Depth 3) — Calculators
> **Error Code:** `P11_E042`
> **Screen:** DropCalc
> **Run:** `index.html?src=tests/d2r-oracle/test-042-calc-nodrop-players.md`

## Description

adjustedNoDrop = (nodrop/total)^(1+(p-1)/2)*total

## Expected Result

```
P1 vs P3 vs P7 NoDrop values correct
```
...
```js
/**
 * D2R Oracle Test — NoDrop with Players Setting
 * Layer: P11 | Error Code: P11_E042
...
function main(params) {
  const testId = '042';
  const errorCode = 'P11_E042';
  const testName = 'calc-nodrop-players';
  const layer = { prime: 11, depth: 3, name: 'Calculators' };
  const testSrc = 'tests/d2r-oracle/test-042-calc-nodrop-players.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: NoDrop with Players Setting');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[042] Running drop calculator formulas');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[042] Drop calculator formulas verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'P1 vs P3 vs P7 NoDrop values correct';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-041-calc-mf-diminishing.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 041</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-043-calc-tc-resolution.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 043 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 042</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P11 \u00B7 Depth 3</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">NoDrop with Players Setting</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">adjustedNoDrop = (nodrop/total)^(1+(p-1)/2)*total</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P11_E042</code></div>' +
        '</div>' +
        
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">P1 vs P3 vs P7 NoDrop values correct</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-042-calc-nodrop-players.md`*
