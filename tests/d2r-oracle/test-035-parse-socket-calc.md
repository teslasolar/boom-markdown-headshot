# Test 035: Parse Socket Calc Query

> **Layer P5** (Depth 2) — Parser / NLP
> **Error Code:** `P5_E035`
> **Screen:** Parser
> **Run:** `index.html?src=tests/d2r-oracle/test-035-parse-socket-calc.md`

## Description

NLP identifies "larzuk crystal sword normal" as SOCKET_CALC

## Query Input

```
larzuk crystal sword normal
```

## Expected Result

```
type: SOCKET_CALC, item: "Crystal Sword", difficulty: "Normal"
```
...
```js
/**
 * D2R Oracle Test — Parse Socket Calc Query
 * Layer: P5 | Error Code: P5_E035
...
function main(params) {
  const testId = '035';
  const errorCode = 'P5_E035';
  const testName = 'parse-socket-calc';
  const layer = { prime: 5, depth: 2, name: 'Parser / NLP' };
  const testSrc = 'tests/d2r-oracle/test-035-parse-socket-calc.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Parse Socket Calc Query');
    ...
    const query = "larzuk crystal sword normal";
    if (typeof MDRUN !== 'undefined') MDRUN.info('[035] Parsing query: ' + JSON.stringify(query));
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[035] Parser classification verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'type: SOCKET_CALC, item: "Crystal Sword", difficulty: "Normal"';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-034-parse-item-eval.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 034</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-036-parse-ambiguous-item.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 036 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 035</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P5 \u00B7 Depth 2</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Parse Socket Calc Query</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">NLP identifies "larzuk crystal sword normal" as SOCKET_CALC</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P5_E035</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">larzuk crystal sword normal</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">type: SOCKET_CALC, item: "Crystal Sword", difficulty: "Normal"</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-035-parse-socket-calc.md`*
