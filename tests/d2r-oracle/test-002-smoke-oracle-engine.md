# Test 002: Oracle Engine Instantiates

> **Layer P2** (Depth 0) — Foundation / Smoke
> **Error Code:** `P2_E002`
> **Screen:** Engine
> **Run:** `index.html?src=tests/d2r-oracle/test-002-smoke-oracle-engine.md`

## Description

Verify oracle.ts query engine creates instance

## Expected Result

```
Oracle instance with all data loaded
```
...
```js
/**
 * D2R Oracle Test — Oracle Engine Instantiates
 * Layer: P2 | Error Code: P2_E002
...
function main(params) {
  const testId = '002';
  const errorCode = 'P2_E002';
  const testName = 'smoke-oracle-engine';
  const layer = { prime: 2, depth: 0, name: 'Foundation / Smoke' };
  const testSrc = 'tests/d2r-oracle/test-002-smoke-oracle-engine.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Oracle Engine Instantiates');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[002] Testing Oracle engine init');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[002] Engine instantiation verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'Oracle instance with all data loaded';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-001-smoke-cli-entry.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 001</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-003-smoke-data-load.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 003 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 002</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P2 \u00B7 Depth 0</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Oracle Engine Instantiates</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Verify oracle.ts query engine creates instance</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Screen:</strong> <span style="color: #e5e7eb;">Engine</span></div>' +
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P2_E002</code></div>' +
        '</div>' +
        
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Oracle instance with all data loaded</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-002-smoke-oracle-engine.md`*
