# Test 001: CLI Entry Point Loads

> **Layer P2** (Depth 0) — Foundation / Smoke
> **Error Code:** `P2_E001`
> **Screen:** CLI
> **Run:** `index.html?src=tests/d2r-oracle/test-001-smoke-cli-entry.md`

## Description

Verify index.ts CLI entry point initializes without error

## Expected Result

```
CLI prompt appears with no crash
```
...
```js
/**
 * D2R Oracle Test — CLI Entry Point Loads
 * Layer: P2 | Error Code: P2_E001
...
function main(params) {
  const testId = '001';
  const errorCode = 'P2_E001';
  const testName = 'smoke-cli-entry';
  const layer = { prime: 2, depth: 0, name: 'Foundation / Smoke' };
  const testSrc = 'tests/d2r-oracle/test-001-smoke-cli-entry.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: CLI Entry Point Loads');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[001] MDRUN runtime detected');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[001] CLI entry point verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'CLI prompt appears with no crash';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-002-smoke-oracle-engine.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 002 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 001</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P2 \u00B7 Depth 0</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">CLI Entry Point Loads</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Verify index.ts CLI entry point initializes without error</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P2_E001</code></div>' +
        '</div>' +
        
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">CLI prompt appears with no crash</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-001-smoke-cli-entry.md`*
