# Test 061: Runeword Matcher Basic

> **Layer P11** (Depth 3) — Calculators
> **Error Code:** `P11_E061`
> **Screen:** RunewordFinder
> **Run:** `index.html?src=tests/d2r-oracle/test-061-calc-runeword-match.md`

## Description

Given runes, find all possible runewords

## Query Input

```
I have Ber Jah Ist Mal Um
```

## Expected Result

```
Enigma, CoH (needs Dol), Duress possibilities
```
...
```js
/**
 * D2R Oracle Test — Runeword Matcher Basic
 * Layer: P11 | Error Code: P11_E061
...
function main(params) {
  const testId = '061';
  const errorCode = 'P11_E061';
  const testName = 'calc-runeword-match';
  const layer = { prime: 11, depth: 3, name: 'Calculators' };
  const testSrc = 'tests/d2r-oracle/test-061-calc-runeword-match.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Runeword Matcher Basic');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[061] Testing runeword matcher');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[061] Runeword finder verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'Enigma, CoH (needs Dol), Duress possibilities';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-060-calc-fbr-paladin.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 060</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-062-calc-runeword-base.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 062 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 061</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P11 \u00B7 Depth 3</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Runeword Matcher Basic</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Given runes, find all possible runewords</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P11_E061</code></div>' +
        '</div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px; margin-bottom: 12px;">' +
          '<strong style="color: #60a5fa;">Query:</strong> <code style="color: #34d399;">I have Ber Jah Ist Mal Um</code></div>' +
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">Enigma, CoH (needs Dol), Duress possibilities</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-061-calc-runeword-match.md`*
