# Test 020: Cube Recipes JSON Valid

> **Layer P3** (Depth 1) — Data Integrity
> **Error Code:** `P3_E020`
> **Screen:** Data
> **Run:** `index.html?src=tests/d2r-oracle/test-020-data-cube-recipes.md`

## Description

All cube recipes have inputs, outputs, and category

## Expected Result

```
All cube recipes pass validation
```
...
```js
/**
 * D2R Oracle Test — Cube Recipes JSON Valid
 * Layer: P3 | Error Code: P3_E020
...
function main(params) {
  const testId = '020';
  const errorCode = 'P3_E020';
  const testName = 'data-cube-recipes';
  const layer = { prime: 3, depth: 1, name: 'Data Integrity' };
  const testSrc = 'tests/d2r-oracle/test-020-data-cube-recipes.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Cube Recipes JSON Valid');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[020] Validating data file manifest');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[020] Data manifest validated: ' + dataFiles.length + ' files');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'All cube recipes pass validation';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-019-data-skills-schema.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 019</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-021-data-mercenaries.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 021 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 020</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P3 \u00B7 Depth 1</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Cube Recipes JSON Valid</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">All cube recipes have inputs, outputs, and category</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P3_E020</code></div>' +
        '</div>' +
        
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">All cube recipes pass validation</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-020-data-cube-recipes.md`*
