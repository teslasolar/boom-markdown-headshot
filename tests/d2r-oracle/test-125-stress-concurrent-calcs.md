# Test 125: Stress: Concurrent Calculations

> **Layer P127** (Depth 5) — Full Integration / Edge Cases
> **Error Code:** `P127_E125`
> **Screen:** Stress
> **Run:** `index.html?src=tests/d2r-oracle/test-125-stress-concurrent-calcs.md`

## Description

Run all calculators simultaneously

## Expected Result

```
No race conditions or corrupt results
```
...
```js
/**
 * D2R Oracle Test — Stress: Concurrent Calculations
 * Layer: P127 | Error Code: P127_E125
...
function main(params) {
  const testId = '125';
  const errorCode = 'P127_E125';
  const testName = 'stress-concurrent-calcs';
  const layer = { prime: 127, depth: 5, name: 'Full Integration / Edge Cases' };
  const testSrc = 'tests/d2r-oracle/test-125-stress-concurrent-calcs.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Stress: Concurrent Calculations');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[125] Running stress test');
    ...
    if (performance.memory) {
      ...
      if (typeof MDRUN !== 'undefined') MDRUN.info('[125] Heap: ' + mb.toFixed(1) + 'MB');
    }
    if (typeof MDRUN !== 'undefined') MDRUN.success('[125] Stress test passed in ' + elapsed.toFixed(0) + 'ms');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = 'No race conditions or corrupt results';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-124-stress-rapid-queries.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 124</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-126-stress-memory-data-load.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 126 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 125</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P127 \u00B7 Depth 5</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Stress: Concurrent Calculations</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Run all calculators simultaneously</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Screen:</strong> <span style="color: #e5e7eb;">Stress</span></div>' +
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P127_E125</code></div>' +
        '</div>' +
        
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          '<strong style="color: #60a5fa;">Expected:</strong>' +
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">No race conditions or corrupt results</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-125-stress-concurrent-calcs.md`*
