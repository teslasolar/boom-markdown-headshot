# Test 097: Cube: Rune Upgrade Path

> **Layer P31** (Depth 4) — Query Screens
> **Error Code:** `P31_E097`
> **Screen:** CubeRecipes
> **Run:** `index.html?src=tests/d2r-oracle/test-097-screen-cube-upgrade-runes.md`

## Description

Full rune upgrade chain with costs
...
```js
/**
 * D2R Oracle Test — Cube: Rune Upgrade Path
 * Layer: P31 | Error Code: P31_E097
...
function main(params) {
  const testId = '097';
  const errorCode = 'P31_E097';
  const testName = 'screen-cube-upgrade-runes';
  const layer = { prime: 31, depth: 4, name: 'Query Screens' };
  const testSrc = 'tests/d2r-oracle/test-097-screen-cube-upgrade-runes.md';
  ...
  try {
    ...
    if (typeof MDRUN !== 'undefined') {
      MDRUN.info('[TEST ' + testId + '] ' + errorCode + ' Starting: Cube: Rune Upgrade Path');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.info('[097] Testing cube recipes');
    ...
    if (typeof MDRUN !== 'undefined') MDRUN.success('[097] Cube recipes verified');

    result.pass = true;
    result.assertions = assertions || [];
    result.output = '256 Um + 8 gems. Do not do this.';
  ...
  navHtml += '<a href="?src=tests/d2r-oracle/test-096-screen-merc-a1-faith.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">&larr; Test 096</a>';
  navHtml += '<a href="?src=' + runnerSrc + '" style="color: #fbbf24; text-decoration: none; font-size: 0.9em;">Test Runner</a>';
  navHtml += '<a href="?src=tests/d2r-oracle/test-098-screen-cube-reroll-gc.md" style="color: #60a5fa; text-decoration: none; font-size: 0.9em;">Test 098 &rarr;</a>';
  navHtml += '</div>';
    ...
    '<div style="max-width: 800px; margin: 40px auto; font-family: \'Courier New\', monospace; color: #e5e7eb;">' +
      '<div style="background: #1a1a2e; border: 2px solid ' + statusColor + '; border-radius: 8px; padding: 24px;">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">' +
          '<h1 style="margin: 0; color: ' + statusColor + '; font-size: 1.5em;">' + status + ' \u2014 Test 097</h1>' +
          '<span style="background: #312e81; color: #a5b4fc; padding: 4px 12px; border-radius: 12px; font-size: 0.85em;">P31 \u00B7 Depth 4</span>' +
        '</div>' +
        '<h2 style="color: #fbbf24; margin: 0 0 8px 0; font-size: 1.2em;">Cube: Rune Upgrade Path</h2>' +
        '<p style="color: #9ca3af; margin: 0 0 16px 0;">Full rune upgrade chain with costs</p>' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">' +
          ...
          '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
            '<strong style="color: #60a5fa;">Error Code:</strong> <code style="color: #fbbf24;">P31_E097</code></div>' +
        '</div>' +
        ...
        '<div style="padding: 12px; background: #111827; border-radius: 4px;">' +
          ...
          '<pre style="color: #e5e7eb; margin: 4px 0 0 0; white-space: pre-wrap;">256 Um + 8 gems. Do not do this.</pre></div>' +
        errorHtml +
        assertHtml +
        navHtml +
...
*Run this test: `index.html?src=tests/d2r-oracle/test-097-screen-cube-upgrade-runes.md`*
